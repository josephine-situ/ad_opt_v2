...
This document describes how the pipeline builds time-series CV folds and which config knobs control behavior. Implementation lives in [`cv.py`](../utils/cv.py); tournament wiring is in [`modeling.py`](../utils/modeling.py).
...
Example block (see `<course>/opt_results/campaign/default/campaign_config.json`):
...
- Default grids live in [`train_specs.py`](../utils/train_specs.py) (ridge `alpha` 10–100; XGB/RF `max_depth` 2–3, `n_estimators` ≤ 20). Rationale and feature-selection context: [feature_selection_and_modeling.md](feature_selection_and_modeling.md).
...
| Symbol | File | Role |
|--------|------|------|
| `time_series_cv_folds` | `utils/cv.py` | Expanding-window fold generation |
| `effective_min_train_days` | `utils/cv.py` | Train-day floor from fraction / absolute min |
| `_validation_kw` | `utils/cv.py` | Config → fold kwargs |
| `cross_validate_model` | `utils/cv.py` | Fit/score loop over folds |
| `run_tournament` | `utils/modeling.py` | Logs fold preview; CV per candidate |
