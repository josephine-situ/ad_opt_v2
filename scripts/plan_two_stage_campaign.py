...
def main() -> None:
    ...
    setup_tee_logging(log_file=None, default_log_prefix=f"plan_two_stage_{config.course}")
    ...
    total_budget = args.budget or float(
        COURSE_CONFIG.get(config.course, {}).get("campaign_budget", 400.0)
    )
    ...
    if args.skip_stage1:
        ...
        print(
            f"[stage1] Keyword-set selection {window_start.date()} → {window_end.date()}"
        )
