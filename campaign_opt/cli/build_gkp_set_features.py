...
from campaign_opt.paths import GKP_DIR, data_path
...
def main() -> None:
    ...
    if not GKP_DIR.exists():
        print(f"[Warn] GKP dir missing: {GKP_DIR}. Run pull_input_data with keyword_planning dataset.")
        print("       Writing semantic-only keyword-set features.")

    table = build_keyword_set_feature_table(COURSE)
    out = data_path("processed", "keyword-set-features.csv")
