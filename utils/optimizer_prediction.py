...
def _predict_levels_sklearn(model: Any, rows: pd.DataFrame, config: CampaignOptConfig) -> np.ndarray:
    from utils.training_matrix import ensure_tree_segment_features, prep_xy

    target = config.target
    feature_cols = _optimizer_feature_cols(model, config)
    sub = ensure_tree_segment_features(rows.copy())
    if target not in sub.columns:
        sub[target] = 0.0
    X, _ = prep_xy(sub, target, feature_cols)
...
def predict_levels_optimizer(
    ...
    *,
    ...
    if not config.evaluation.apply_observed_budget_floor:
        ...
        return _predict_levels_sklearn(model, rows, config)
    ...
    if isinstance(model, EnsembleModel):
        ...
        raw = _predict_levels_sklearn(model, rows, config)
